...
/**
 * Get all supported versions as an array, optionally filtered by OS type
 * Order matters! Oldest => latest
 * @param {string} osType - Optional OS type to filter by. If not provided, uses current OS
 * @param {boolean} includeAll - If true, returns all versions regardless of OS support
 * @returns {string[]} - Array of version strings
 */
const getSupportedVersions = (osType = null, includeAll = false) => {
    const allVersions = Object.keys(ZXP_VERSION);
    
    if (includeAll) {
        return allVersions;
    }
    
    const targetOs = osType || getCurrentOs();
    return allVersions.filter(version => isOsSupportedByVersion(version, targetOs));
};
...
module.exports.getCurrentOs = getCurrentOs;
